...
export function AvatarSelector() {
    ...
    //This page could do with a refactor!! change alot of the buttons and conditional logic in to sepereate components
    ...
    function sell() {
      if(currentUser.owned_avatar_ids.length > 1 && Number(currentUser.avatar_uri) !== currentAvatar) {
      const newBoughtAvatars = currentUser.owned_avatar_ids.filter(avatar => avatar !== currentAvatar)
      const newStats = {...currentUser.stats, coins: currentUser.stats.coins + 50}
        ...
        <ImageBackground source={require('../assets/images/wood.jpg')} style={styles.background} resizeMode="cover">
            ...
            <View style={{flexDirection: 'row', width: '90%'}}>
              ...
              <Pressable style={[styles.button, avatarChecker()? styles.disabledButton : null]} onPress={sell}>
                  <Text style={styles.text}>Sell: 50 Gold</Text>
              </Pressable> }
            </View>
            ...
            <View style={styles.selectButton}>
              {avatarChecker() && Number(currentUser.avatar_uri) !== currentAvatar?
            ...
            <View style={styles.overlay}>
              <Text style={styles.text}>{Number(currentUser.avatar_uri) !== currentAvatar?'Must Have At Least One Avatar' : 'Cant Sell Current Avatar'}</Text>
            </View> : null}
        </ImageBackground>
    </View>
  );
